...
# ── CONFIGURE THESE ──────────────────────────────────────────
XML_DIR    = 'C:/Users/Admin/Desktop/BAKAULARA DARBS/dataset/uav/val/xml'   # folder containing your .xml files
IMAGES_DIR = 'C:/Users/Admin/Desktop/BAKAULARA DARBS/dataset/uav/val/img'            # folder containing your images
OUTPUT_DIR = 'C:/Users/Admin/Desktop/BAKAULARA DARBS/dataset/uav/val/txt'            # where YOLO .txt files will be saved
...
for xml_file in sorted(os.listdir(XML_DIR)):
    ...
    if is_pascal_voc:
        # One XML file = one image
        filename, img_w, img_h, boxes = parse_pascal_voc(tree)
        ...
        # One XML file may contain multiple images (DUT custom format)
        records = parse_dut_custom(tree)
        for (filename, img_w, img_h, boxes) in records:
            stem = os.path.splitext(filename)[0]
            out_path = os.path.join(OUTPUT_DIR, stem + '.txt')
            write_yolo_label(out_path, boxes, img_w, img_h)
            converted += 1
